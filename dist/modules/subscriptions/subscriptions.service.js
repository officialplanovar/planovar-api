...
let SubscriptionsService = SubscriptionsService_1 = class SubscriptionsService {
    ...
    async expireLapsedSubscriptions() {
        ...
        const candidates = await this.prisma.vendorSubscription.findMany({
            where: {
                plan: { tier: { not: client_1.SubscriptionTier.BASIC } },
                OR: [
                    { status: client_1.SubscriptionStatus.TRIALING, trialEndsAt: { lt: now } },
                    {
    ...
    priceInBillingCurrency(monthly, yearly) {
    ...
    async ensureBasicSubscription(vendorId) {
        const select = {
            id: true,
            status: true,
            billingCycle: true,
        ...
        const existing = await this.prisma.vendorSubscription.findFirst({
            where: {
                vendorId,
                status: client_1.SubscriptionStatus.ACTIVE,
                plan: { tier: client_1.SubscriptionTier.BASIC },
            },
            orderBy: { createdAt: 'desc' },
            select,
        });
        ...
        const basicPlan = await this.prisma.subscriptionPlan.findUnique({
            where: { tier: client_1.SubscriptionTier.BASIC },
            select: { id: true },
        });
        if (!basicPlan) {
            throw new common_1.NotFoundException('BASIC plan is not configured');
        }
        const now = new Date();
    ...
    async subscribe(userId, dto, deviceId) {
        ...
        const newPrice = billingCycle === client_1.BillingCycle.YEARLY
            ? pricing.priceYearly
    ...
    async isTrialEligible(vendorId, phone, deviceId) {
        ...
        const priorPaid = await this.prisma.vendorSubscription.findFirst({
            where: { vendorId, plan: { tier: { not: client_1.SubscriptionTier.BASIC } } },
            select: { id: true },
        });
        return !priorPaid;
    }
    async proratedCredit(vendorId, now) {
        ...
        if (!current)
            return 0;
        const periodMs = current.currentPeriodEnd.getTime() - current.currentPeriodStart.getTime();
        ...
        const ngnPeriodPrice = current.billingCycle === client_1.BillingCycle.YEARLY
    ...
    async verifyPayment(userId, reference) {
        ...
        const meta = verified.raw?.metadata;
        const metaMatchesVendor = meta?.type === 'subscription' && meta?.vendorId === vendor.id;
        ...
        if (!byReference && !metaMatchesVendor) {
            throw new common_1.BadRequestException('Payment reference does not match a pending subscription for this account');
        }
...
], SubscriptionsService.prototype, "expireLapsedSubscriptions", null);
