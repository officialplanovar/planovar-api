...
let VendorsService = class VendorsService {
    ...
    async claimVendorIntent(userId) {
        ...
        if (user.clientProfile?.onboardingComplete) {
            throw new common_1.ForbiddenException('This account is registered as a client. Use a different account to sell on Planovar.');
        }
    ...
    async submitKyc(userId, dto) {
        ...
        if (vendor.businessType === 'LICENSED' && !dto.businessRegDocumentUrl) {
            throw new common_1.BadRequestException('A business registration document is required for registered businesses');
        }
        const updated = await this.prisma.vendorProfile.update({
            where: { id: vendor.id },
            data: {
