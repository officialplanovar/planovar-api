...
const ALL_EVENT_TYPES = Object.values(client_1.EventType);
function eventTypesForIndex(eventTypes) {
...
let TypesenseSyncService = TypesenseSyncService_1 = class TypesenseSyncService {
    ...
    async reconcileSchema(schema) {
        try {
            ...
            this.logger.log(`Typesense: added field(s) to ${schema.name}: ${missing
                .map((f) => f.name)
                .join(', ')}`);
        }
        catch (err) {
    ...
    async reindexVendorAndListings(vendorId) {
        try {
