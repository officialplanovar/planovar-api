...
/**
 * Better Auth mounts its routes as Express middleware, bypassing NestJS
 * controllers and therefore Swagger's auto-scanner. This function manually
 * injects all relevant auth paths into the generated document so they appear
 * in /docs and can be tested interactively.
 */
export function injectBetterAuthPaths(document: OpenAPIObject): void {
  ...
  document.paths['/api/auth/sign-up/email'] = {
    post: {
      ...
      description:
        'Creates a new user account. Pass `role: "CLIENT"` from the client app and `role: "VENDOR"` from the vendor app. An OTP is sent to the email address for verification.',
      requestBody: {
        required: true,
        ...jsonContent({
          type: 'object',
  ...
  document.paths['/api/auth/sign-in/social'] = {
    post: {
      ...
      description:
        'Returns a redirect URL to Google. In the Flutter apps this is handled natively via the google_sign_in package — this endpoint is primarily used by the admin web console.',
      requestBody: {
        required: true,
        ...jsonContent({
          type: 'object',
