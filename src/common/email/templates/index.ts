// ─── Planovar email design system ────────────────────────────────────────────
//
// Brand (from the Planovar logo):
//   gradient   magenta #C44AE1 → indigo #5B50F0 → blue #2D9CFD
//   navy       #0F0D2E (dark surface)   gold #D9A441 (sparkle / tagline)
//   tagline    "Plan better. Celebrate bigger."
//
// Graphics are CSS-only (confetti band, gradient bars, sparkles) so they render
// even when image loading is blocked. The logo is an <img> read from
// EMAIL_LOGO_URL (host the PNG on R2/CDN); until it's set, a styled wordmark
// fallback keeps the header on-brand.
...
// Logo URLs — served by the API itself from planovar-api/public/branding/
// (see the README there for file names). EMAIL_LOGO_URL overrides (e.g. CDN).
const assetBase = () =>
  (process.env.API_BASE_URL ?? 'http://localhost:3000').replace(/\/$/, '');
const logoDarkUrl = () =>
  process.env.EMAIL_LOGO_URL ?? `${assetBase()}/branding/planovar-logo-dark.png`;
const logoLightUrl = () => `${assetBase()}/branding/planovar-logo-light.png`;

/**
 * Header logo with light/dark adaptability:
 * both variants are embedded; `prefers-color-scheme` CSS swaps them in clients
 * that support it (Apple Mail, Outlook). Default shown = dark-bg variant, which
 * sits on the navy header and reads fine in both modes. A styled wordmark is
 * the alt-text fallback when images are blocked.
 */
const logoBlock = () => `
...
/** CSS confetti band — the celebration motif from the logo, image-free. */
const confetti = () => `
...
export const emailTemplates = {
  ...
  bookingRequest(data: { clientName: string; vendorName: string; listingTitle: string; eventDate: string; eventLocation: string; requirements?: string }) {
    return {
  ...
  bookingConfirmed(data: { clientName: string; vendorName: string; listingTitle: string; eventDate: string; eventLocation: string }) {
    return {
  ...
  bookingCancelled(data: { recipientName: string; listingTitle: string; eventDate: string; cancelledBy: string; reason?: string }) {
    return {
  ...
  quoteReceived(data: { clientName: string; vendorName: string; listingTitle: string; amount: string; validUntil: string }) {
    return {
  ...
  quoteAccepted(data: { vendorName: string; clientName: string; listingTitle: string; amount: string }) {
    return {
  ...
  reviewRequested(data: { clientName: string; vendorName: string; listingTitle: string }) {
    return {
  ...
  disputeOpened(data: { adminSubject: string; bookingId: string; raisedBy: string; reason: string }) {
    return {
  ...
  welcome(data: { firstName: string; role: 'client' | 'vendor' }) {
